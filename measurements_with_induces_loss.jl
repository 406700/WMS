...
mod_L_prime_new  = load(mod_path_new[1:end-3] * "_L_prime.jld2")
...
mod_L_prime_new2 = load(mod_path_loss_new[1:end-3] * "_L_prime.jld2")
...
for key in ["1", "2"]
    ...
    x3, y3 = average_vector_in_chunks(mod_x_new[key], mod_L_prime_new[key]./mod_L_new[key], window_size)
    axs[2].plot(x3, y3, label="")

    # Second figure data (L from mod_L)
    x4, y4 = average_vector_in_chunks(mod_x_new2[key], mod_L_prime_new2[key]./mod_L_new2[key], window_size)
    axs[2].plot(x4, y4, label="loss")
...
fig.tight_layout()
fig.savefig("spie_figures/loss_comparison_alpha_20252.png", dpi=600)
...
fig2.tight_layout()
fig2.savefig("spie_figures/loss_comparison_T_20252.png", dpi=600)
